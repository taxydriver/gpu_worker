...
def test_rehydrated_retirement_accepts_hashed_legacy_tunnel_unit_filename(
    ...
    stage_data = json.loads(active.stage_receipt.read_text())
    stage_data.pop("tunnel_unit_artifact")
    active.stage_receipt.write_text(json.dumps(stage_data, sort_keys=True, indent=2) + "\n")
    tunnel_unit_path = layout.systemd_root / contract.tunnel_unit
    assert tunnel_unit_path.resolve() == legacy.resolve()
